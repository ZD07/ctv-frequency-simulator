...
st.caption(
    ...
    "Because there's no shared identity layer across streaming environments, "
    "advertisers can't control cross-platform frequency directly — this tool makes the problem visible."
)
...
headers = ["Platform", "Universe (HHs)", "Budget (£)", "CPM (£)", "Freq Cap"]
...
st.caption(
    ...
    "Pre-populated with UK estimates from BARB and Ofcom data (2024)."
)
...
n_sims = col2.selectbox(
    "Simulation precision (higher = slower but more accurate)",
...
if run:
    ...
    if not results:
        ...
        colours = [
            "#ef4444" if xi > target_cap else "#3b82f6"
            for xi in x
        ]
